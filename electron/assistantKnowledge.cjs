...
const CATALOG = [
  ...
  { id: 'settings', labels: ['Personlige indstillinger', 'Personal settings', 'Omat asetukset'], topic: /dashboard|widget|tema|theme|indstilling|settings|teema|asetus/u, view: 'hub', fields: 'own dashboard widget preferences, active theme and available themes; no other users preferences' },
    ...
    for (const team of principal.teams) {
      ...
      for (const module of [...CATALOG, ...PLANNING_MODULES].filter(module => ids.includes(module.id))) {
        ...
        if (module.id === 'shifts') {
          ...
          add('active-theme', 'Theme / Tema / Teema', clean(readTeam(team, `active-theme-${principal.email}`)) || 'default')
          for (const theme of array(readTeam(team, 'custom-themes'))) if (theme?.id) add(`theme:${theme.id}`, theme.name, `${clean(theme.name)}\n${clean(theme.createdBy)}\n${clean(theme.radius)}\n${['background', 'foreground', 'primary', 'accent'].map(key => `${key}: ${clean(theme.colors?.[key], 120)}`).join('\n')}`)
