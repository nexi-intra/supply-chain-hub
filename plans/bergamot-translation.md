# Bergamot neural oversættelse (da↔en↔fi) — offline i appen
...
- **Modeller på det delte drev** (`<datamappe>/translation-models/daen|enda|enfi|fien/`): én download tjener alle klienter, app-zippen vokser ikke, og modeller kan opdateres uden ny app-version. Dansk↔finsk pivoterer gennem engelsk.
...
- [x] `scripts/download-translation-models.cjs` — henter da↔en og en↔fi fra Mozilla Remote Settings, verificerer komprimerede og udpakkede SHA-256-checksummer og installerer dem centralt. Finsk version 3.0 er installeret på Supply Chain Hub-fællesdrevet og valideret med appens Bergamot-runtime.
