...
export function AssistantChatWindow({ language, width, onWidthChange, onMinimize, onClose, status, ready, settings, children }: {
  ...
  return <section aria-label={t.title} data-assistant-window className="fixed bottom-5 right-5 z-40 flex flex-col overflow-hidden rounded-lg border border-border bg-popover text-popover-foreground shadow-xl" style={{ width: `min(${CHAT_WIDTHS[width]}px, calc(100vw - 24px))`, height: 'min(620px, calc(100dvh - 40px))' }}>
    <header className="relative flex h-14 shrink-0 items-center gap-2 bg-[#142052] px-3 text-white">
      ...
      <details className="group">
        ...
        <div className="absolute right-3 top-12 z-10 w-56 rounded-md border bg-popover p-3 text-popover-foreground shadow-lg">
